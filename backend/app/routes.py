...
def get_admin_analytics():
    ...
    total_students = db.users.count_documents(student_filter)
    ...
    courses = list(db.courses.find(course_filter))
    course_analytics = []
    for course in courses:
        course_id_str = str(course['_id'])
        students_in_course = db.users.count_documents({"role": "student", "course_id": course_id_str})
        attendance_in_course = db.attendance.count_documents({"course_id": course_id_str})
        distinct_dates_in_course = db.attendance.distinct("date", {"course_id": course_id_str})
        possible_in_course = students_in_course * len(distinct_dates_in_course)
    ...
    return jsonify({"totalStudents": total_students, "overallAttendancePercentage": round(overall_percentage, 2), "courseAnalytics": course_analytics})
...
def mark_attendance():
    ...
    if match_index is not None:
        ...
        db.attendance.update_one({"student_id": matched_student['_id'], "course_id": course_id, "date": today}, {"$set": {"status": "Present"}}, upsert=True)
...
def get_teacher_course_analytics(course_id):
    ...
    students_in_course = db.users.count_documents({"role": "student", "course_id": course_id})
    
    # Filter for dates within the selected month
    attendance_filter = {"course_id": course_id, "date": {"$regex": f"^{month_str}"}}
    total_attendance_records = db.attendance.count_documents(attendance_filter)
    distinct_dates = db.attendance.distinct("date", attendance_filter)
    
    total_possible_attendance = students_in_course * len(distinct_dates)
    overall_percentage = (total_attendance_records / total_possible_attendance) * 100 if total_possible_attendance > 0 else 0

    seven_days_ago = datetime.datetime.now() - datetime.timedelta(days=7)
    recent_dates_all = sorted(db.attendance.distinct("date", {"course_id": course_id}), reverse=True)
    recent_dates = [d for d in recent_dates_all if d >= seven_days_ago.strftime("%Y-%m-%d")][:7]

    daily_stats = []
    for date_str in recent_dates:
        present_count = db.attendance.count_documents({"course_id": course_id, "date": date_str, "status": "Present"})
        absent_count = students_in_course - present_count
    ...
    return jsonify({
        ...
        "totalClasses": len(distinct_dates)
    })
    ...
    if view_mode == 'weekly':
        ...
        for date_str in all_session_dates:
            if date_str and date_str.startswith(month_str):
                if date_str in student_present_dates: present_count += 1
                else: absent_count += 1
        ...
        for date_str in sorted(all_session_dates, reverse=True):
            daily_log.append({"date": date_str, "status": "Present" if date_str in student_present_dates else "Absent"})
...
def get_all_students():
    # UPDATED to include course_id
    students = list(db.users.find({"role": "student"}, {"name": 1, "roll_no": 1, "course_id": 1}))
    for student in students: student['_id'] = str(student['_id'])
...
def get_student_analytics(student_id):
    # ... (this function remains the same)
    try: student_obj_id = ObjectId(student_id)
    except Exception: return jsonify(msg="Invalid student ID format"), 400
...
@jwt_required(optional=True) # <-- Add optional=True here
def get_my_profile():
    current_user_id = get_jwt_identity()
    ...
    # If the user is a student, also fetch their course name
    if user['role'] == 'student' and 'course_id' in user:
...
def update_student(student_id):
    try:
        ...
        if 'roll_no' in data:
            update_data['roll_no'] = data['roll_no']
        # NEW: Handle course change
...
def get_course_report(course_id):
    ...
    # 2. Get all distinct dates for that course and month
    ...
    for student in students:
        ...
        # Get all present records for this student for the relevant dates
...
def get_full_school_report():
    ...
    if not month_str or not course_ids_str:
        return jsonify(msg="Month and course parameters are required"), 400

    course_ids = course_ids_str.split(',')
    
    # 1. Get all courses and students
    ...
    # 3. Create a sheet for each course
    for course in courses:
        course_id = str(course['_id'])
        course_name = course['name']
        ws = wb.create_sheet(title=course_name[:31]) # Sheet titles have a 31-char limit
        ...
        students_in_course = [s for s in students if s.get('course_id') == course_id]
